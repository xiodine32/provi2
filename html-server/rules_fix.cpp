...
std::string say_time_left(time_t theTime){
    ...
    str << (contest_time*60-(theTime-time_start));
    return str.str();
}
...
//	char towrite[16384]={"ERROR"};memset(towrite,0,sizeof(towrite));strcpy(towrite,"ERROR");
    ...
    toWrite<<say_time_left(theTime)<<"|"<<echipe<<"|"<<probleme<<"|";
...
void tick(time_t theTime2){
	...
	if (theTime != theTime2) {
		...
		fprintf(a,"%s",say_drawables(theTime2).c_str());
		fclose(a);
	}

	static int lleft=-1;
	int left= contest_time * 60 - (theTime2 - time_start);
	if ((theTime2 - time_start) > 21 * 60){
		for (int i=1;i<=echipe;i++)
...
void load_backup(time_t theTime){
	...
	char *mover;
	mover = strtok(pd,"|");
	long time_left = - contest_time * 60 + atol(mover);
	time_start = (theTime + time_left);
	std::cout<<"time start: "<< time_start << " clock now(): "<< theTime << " "<<time_left<<" time left: "<< say_time_left(theTime)<<"\n";
