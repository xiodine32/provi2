...
std::string say_time_left(){
    ...
    str << (contest_time*60-(time(NULL)-time_start));
    return str.str();
}
...
void load_backup(){
	...
	time_start = (time(NULL) + time_left);
	std::cout<<"time start: "<< time_start << " clock now(): "<< time(NULL) << " "<<time_left<<" time left: "<< say_time_left()<<"\n";
